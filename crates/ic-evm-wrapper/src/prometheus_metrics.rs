//! どこで: canister の運用メトリクス出力
//! 何を: Prometheus 形式のメトリクスエンコード
//! なぜ: debug_print 依存から脱却し、時系列監視を可能にするため

use ic_metrics_encoder::MetricsEncoder;

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct DropCountSample {
    pub code: u16,
    pub count: u64,
}
...
pub struct PrometheusSnapshot {
    ...
    pub drop_counts: Vec<DropCountSample>,
}
...
#[cfg(test)]
mod tests {
    use super::{encode_prometheus, DropCountSample, PrometheusSnapshot};
    ...
    fn encode_prometheus_includes_expected_metrics() {
        let snapshot = PrometheusSnapshot {
            ...
            drop_counts: vec![DropCountSample { code: 2, count: 9 }],
        };
