use super::{
    ...
    validate_execution_result_sizes, ExecError, InternalTraceActionKind, InternalTraceInspector,
    LogEntry, OpHaltReason, StateDiff, FEE_RECIPIENT, MAX_RETURN_DATA,
};
...
fn internal_trace_selfdestruct_does_not_shift_sibling_trace_ids() {
    ...
    assert_eq!(
        traces.items[3].to_address,
        Some(sibling_target.into_array())
    );
}
