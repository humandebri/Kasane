...
fn sequential_nonces_are_included_across_blocks() {
    ...
    let tx0 = common::build_zero_to_ic_tx_input(0, 2_000_000_000, 1_000_000_000);
    let tx1 = common::build_zero_to_ic_tx_input(1, 2_000_000_000, 1_000_000_000);

    fund_principal(&[0x77]);
    let id0 = chain::submit_tx_in(TxIn::IcSynthetic {
    ...
    let id1 = chain::submit_tx_in(TxIn::IcSynthetic {
        caller_principal: vec![0x77],
        canister_id: vec![0x07],
        tx: tx1,
    })
...
fn nonce_too_low_is_rejected() {
    ...
    chain::submit_tx_in(TxIn::IcSynthetic {
        ...
        canister_id: vec![0x09],
        tx: tx0,
    })
    .expect("submit 0");
    let _ = chain::produce_block(1).expect("produce block");

    let tx0_again = common::build_zero_to_ic_tx_input(0, 3_000_000_000, 1_000_000_000);
    let err = chain::submit_tx_in(TxIn::IcSynthetic {
        caller_principal: vec![0x99],
        canister_id: vec![0x09],
        tx: tx0_again,
    })
    .expect_err("nonce too low");
...
fn replacement_requires_higher_effective_fee() {
    ...
    let low_fee = common::build_zero_to_ic_tx_input(0, 2_000_000_000, 1_000_000_000);
    let high_fee = common::build_zero_to_ic_tx_input(0, 3_000_000_000, 2_000_000_000);

    fund_principal(&[0xaa]);
    let low_id = chain::submit_tx_in(TxIn::IcSynthetic {
        caller_principal: vec![0xaa],
        canister_id: vec![0x0a],
        tx: low_fee,
    })
    .expect("submit low");
    let high_id = chain::submit_tx_in(TxIn::IcSynthetic {
        caller_principal: vec![0xaa],
        canister_id: vec![0x0a],
        tx: high_fee,
    })
