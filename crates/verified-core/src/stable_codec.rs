...
#[cfg_attr(verus_keep_ghost, verus_spec(len => ensures
    ...
    matches!(len, Some(_)) == (
        ...
        && base_len as int + (item_count as int) * (item_len as int) <= usize::MAX as int
    ),
    matches!(len, Some(_)) ==> len.unwrap() as int
        == base_len as int + (item_count as int) * (item_len as int),
))]
pub fn variable_items_len(
    ...
    #[cfg(verus_keep_ghost)]
    proof! {
        assert((item_count as int) * (item_len as int) <= u128::MAX as int)
            by(nonlinear_arith);
        assert((base_len as int) + (item_count as int) * (item_len as int) <= u128::MAX as int)
            by(nonlinear_arith);
    }
    let payload = (item_count as u128) * (item_len as u128);
    let total = (base_len as u128) + payload;
...
#[cfg_attr(verus_keep_ghost, verus_spec(matches => ensures
    matches == (
        item_count <= max_items
        && base_len as int + (item_count as int) * (item_len as int) <= usize::MAX as int
        && actual_len as int == base_len as int + (item_count as int) * (item_len as int)
    ),
))]
...
#[cfg_attr(verus_keep_ghost, verus_spec(len => ensures
    ...
    matches!(len, Some(_)) == (
        ptr_count <= max_ptrs
        && 4 + (ptr_count as int) * 20 <= usize::MAX as int
    ),
    matches!(len, Some(_)) ==> len.unwrap() as int == 4 + (ptr_count as int) * 20,
))]
pub fn prune_journal_len(ptr_count: u32, max_ptrs: u32) -> Option<usize> {
    ...
    #[cfg(verus_keep_ghost)]
    proof! {
        assert((ptr_count as int) * 20 + 4 <= u128::MAX as int) by(nonlinear_arith);
    }
    let total = 4u128 + (ptr_count as u128) * 20u128;
    if total > usize::MAX as u128 {
        None
    } else {
        Some(total as usize)
...
#[cfg_attr(verus_keep_ghost, verus_spec(matches => ensures
    matches == (
        ptr_count <= max_ptrs
        && 4 + (ptr_count as int) * 20 <= usize::MAX as int
        && actual_len as int == 4 + (ptr_count as int) * 20
    ),
))]
...
mod tests {
    ...
    fn prune_journal_len_accounts_for_header_and_ptrs() {
        ...
        assert!(!prune_journal_len_matches(45, 2, 10));
    }
    ...
    fn variable_items_len_checks_count_and_overflow() {
        ...
        assert!(!variable_items_len_matches(69, 4, 2, 32, 10));
        assert_eq!(variable_items_len(4, 11, 32, 10), None);
        assert_eq!(variable_items_len(usize::MAX, 1, 32, 10), None);
        assert_eq!(
            variable_items_len(0, usize::MAX, 1, usize::MAX),
            Some(usize::MAX)
        );
        assert_eq!(variable_items_len(1, usize::MAX, 1, usize::MAX), None);
    }
}
