...
English version: [./indexer-runbook.md](./indexer-runbook.md)
...
- `icp network status --json` で `api_url` を確認できること

### 1.5.1 managed network をクリーン起動
...
```bash
curl -m 2 -sSf http://127.0.0.1:8000/api/v2/status > /dev/null && echo "local api OK"
curl -m 2 -sSf http://127.0.0.1:8080/api/v2/status > /dev/null && echo "icx-proxy OK"
```

### 1.5.2 ローカル接続先の混入チェック
...
```bash
rg -n "8000|IC_HOST|api_url|api/v2/status|http://127\.0\.0\.1" .
```
...
* 対処フロー:
  ...
  - 悪化時のみ upgrade 引数の `query_instruction_soft_limit` / `update_instruction_soft_limit` 調整を検討
...
前提:
- `icp`, `cargo`, `npm`, `python` が使える
- 既存の local managed network を止めて良い（`NETWORK_CLEAN=1` が既定）
...
- canister停止（`icp network stop`）→ retry/backoff が暴走せずログが読みやすい
