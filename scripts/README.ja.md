...
- `scripts/ci_github_equivalent.sh`: `.github/workflows/ci.yml` と `scripts/ci-local.sh` が共有する GitHub 相当チェックの単一ソース
  - Rust の標準品質ゲートとして、specgen 管理の Verus contract target を除く workspace Rust file の `rustfmt --check` と `cargo clippy --workspace --all-targets --all-features -- -D warnings` を含む
