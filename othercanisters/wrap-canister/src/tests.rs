    use super::{
        ...
        GetUnwrapRequirementsArgs, NormalizedDispatchUnwrapRequest, NormalizedSubmitWrapRequest,
        QueueMeta, RequestResult, RequestStatus, StoredRequest, SubmitTxError,
        SubmitWrapRequestArgs, UnwrapReadiness, WrapRequestResult, WrapStoredRequest,
    ...
    fn reset_state() {
        ...
        with_state_mut(|state| {
            let allowed_asset_keys: Vec<_> =
                state.allowed_assets.range(..).map(|entry| entry.key().clone()).collect();
            ...
            let wrap_request_keys: Vec<_> =
                state.wrap_requests.range(..).map(|entry| *entry.key()).collect();
            for key in wrap_request_keys {
                state.wrap_requests.remove(&key);
            }
            let wrap_queue_keys: Vec<_> =
                state.wrap_queue.range(..).map(|entry| *entry.key()).collect();
    ...
    fn replace_allowed_assets_overwrites_previous_entries() {
        ...
        assert!(super::ensure_asset_allowed(
            Principal::self_authenticating(b"asset-a").as_slice()
        )
        .is_ok());
        assert_eq!(
    ...
    fn get_unwrap_requirements_remains_callable_after_asset_delist() {
        ...
        let result = run_ready_future(super::get_unwrap_requirements(GetUnwrapRequirementsArgs {
            asset_id: Principal::from_slice(&vec![2u8; 29]),
    ...
    fn wrap_worker_keeps_processing_delisted_asset_after_acceptance() {
        ...
        with_state(|state| {
            ...
            assert_eq!(req.result.error_code.as_deref(), Some("arg.principal_invalid"));
            assert_ne!(req.result.error_code.as_deref(), Some("asset.not_allowed"));
            assert_eq!(req.result.mint_failed_recoverable, false);
        });
    }
