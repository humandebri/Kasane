...
- `0x00000000000000000000000000000000ffff0003` は ICP query precompile として予約する。
  ...
  - `rpc_eth_call_object_with_query_precompile` は composite query 経由で allowlist 済み query method を `Call::bounded_wait(...).change_timeout(1)` で呼び、raw Candid reply bytes を返す。
...
```mermaid
flowchart TD
  A["Frontend / Wallet"] -->|"update call: submit_ic_tx(record)"| B["Wrapper (ic-evm-gateway)"]
...
この実行基盤を外部に公開する主要な書き込み導線は、crates/ic-evm-gateway/src/lib.rsで定義されたsubmit_* + auto-productionです。これらのupdate callを通じて、内部ではcrates/evm-core/src/chain.rsの実行ロジックが段階的に適用されます。
...
- cycle observer 間隔: migration中 `60s` / 通常 `3600s`（`crates/ic-evm-gateway/src/lib.rs` の `set_timer` 再帰スケジュール）
